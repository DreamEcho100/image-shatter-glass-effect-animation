...
import '//cdnjs.cloudflare.com/ajax/libs/gsap/1.13.2/TweenMax.min.js';
// import { TimelineMax, TweenMax, Cubic } from 'gsap';
...
function imagesLoaded() {
	// canvas.appendChild(image); // Move this line here
	placeImage(false);
	// triangulate();
	// shatter();
}
...
function placeImage(transitionIn = true) {
	...
	// Call triangulate and shatter after the image is placed on the canvas
	triangulate();
	shatter(image);
}
...
function handleMouseDown(event) {
	...
	// Trigger shatter when mouse is clicked
	clickPosition[0] = mouseX;
	clickPosition[1] = mouseY;
	triangulate();
	shatter();
}
