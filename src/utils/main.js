...
const { TWO_PI, store } = (function vars() {
	// Constants
...
const { randomRange, clamp, sign } = (function mathUtils() {
...
function triangulate() {
	...
	// const gridColumns = 10; // Adjust as needed
	// const gridRows = 10; // Adjust as needed

	// /** @type {[number, number][]} */
	// const vertices = [];
	// const stepX = store.imageWidth / (gridColumns - 1);
	// const stepY = store.imageHeight / (gridRows - 1);
	...
	// store.indices = [...new Delaunator(vertices.flat()).triangles];

	// console.log('Vertices:', JSON.stringify(store.vertices, null, 2));
	// console.log('Indices:', JSON.stringify(store.indices, null, 2));
}
...
function shatter(image) {
	...
	for (let i = 0; i < store.indices.length; i += 3) {
		...
		const transitionDuration = randomRange(0.6, 0.9);
		const transitionDurationPadding = 0.2;
		const opacityDuration = transitionDuration * 0.95; //  * 0.95;
		...
		fragment.canvas.style.zIndex = Math.floor(zIndex).toString(); // Math.floor(d).toString();
...
function imagesLoaded() {
	...
	// store.image.onload = function () {
	// 	// Set up image dimensions and triangulate only after the image is fully loaded
	// 	placeImage(false);
	// 	triangulate();
	// 	shatter(store.image);
	// };
...
function setupPlacedImage(transitionIn) {
	...
	if (transitionIn !== false) {
		...
		store.image.addEventListener('animationend', (event) => {
			console.log('event', event);
		});

		const animation = store.image.animate(
			[
				{ transform: 'translateY(-1000px)' }, // From
				{ transform: 'translateY(0)' }, // To
			],
			{
				duration: 750,
				easing: 'ease-out',
				fill: 'both', // This ensures the animation retains its final state
				id: 'transition-y-to-0',
			},
		);

		animation.onfinish = async function (event) {
			console.log('Animation finished!', event);
			if (/** @type {{ id: string } | null} */ (event.target)?.id === 'transition-y-to-0') {
				...
				const screenshotDataURL = await takeScreenshot(containerElement);
				...
				// Append the image element to the container or any desired location
				document.body.appendChild(imageElement);

				// Now you can use the imageElement for further processing, such as shattering it
				// Ensure to update the logic for triangulation, shattering, etc., to work with the imageElement
			}
		};
	}
}
...
export function setup() {
	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		for (let i = 0; i < urls.length; i++) {
			...
			// store.images[i].crossOrigin = 'true';
			// + '?time=' + new Date().valueOf() crossorigin='anonymous'
			/** @type {HTMLImageElement} */ (store.images[i]).onload = function () {
				if (++loaded === 1) {
					imagesLoaded();
					for (let i = 1; i < 4; i++) {
						store.images[i] = new Image();
						store.images[i].crossOrigin = 'anonymous';
						/** @type {HTMLImageElement} */ (store.images[i]).src = urls[i] + '?time=' + new Date().valueOf();
					}
				}
			};
			/** @type {HTMLImageElement} */ (store.images[i]).src = url;
		}
	});
}
